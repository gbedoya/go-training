...
func WorkerPool() {
	...
	// Here we send 5 jobs then close that channel to indicate that's all the work we have.
	for j := 1; j <= 5; j++ {
		jobs <- j
	}
	close(jobs)
	...
	// The program only takes 2 seconds despite doing about 5 seconds of total work
